...
$(function (){
	...
	/*$(".question input[type='text']").focus(function (){
		$(".question li span").hide();
	})
	$(".question input[type='text']").blur(function (){
		if($(".question input[type='text']")==""){
			$(".question li span").show();
		}
	})*/
	var num = 0;
	var num2 = 0;
...
<<<<<<< .mine
		...
		var monthIncomePublic = $("input[name='monthIncomePublic']").val();	
		var monthIncomePrivate = $("input[name='monthIncomePrivate']").val();	
		var housePayType = $("input[name='housePayType']:checked").val();	//房产类型
		var personalDebt = $("input[name='personalDebt']:checked").val();	
		var companyDebt = $("input[name='companyDebt']:checked").val();	
=======
		...
		//第二题
			...
			if(companyType != 1){
				if(shares == '' || shares < 0 || shares > 100 || $.isNumeric(shares) == false){
					$(".loan_succ_tck2").stop();
			        tips('请输入您所在公司的真实所占股份比例，要不测试结果不准确哦~');
					return false;
				}
			}
			else{
			...
			if(housePayType == 1){
				...
				if(localHouseType == 21){			
					if( localHouseTotalPrice == '' || localHouseTotalPrice > 50000 || localHouseTotalPrice < 0  || $.isNumeric(localHouseTotalPrice) == false ){
					    $(".loan_succ_tck2").stop();
			            tips('请填写正确买断房产总价，应该在0-50000（万）元之间');
						return false;
					}
				...
				//月供
					...
					if( localHouseMonthTotalPrice == '' || localHouseMonthTotalPrice > 50000 || localHouseMonthTotalPrice < 0   || $.isNumeric(localHouseMonthTotalPrice) == false ){
						$(".loan_succ_tck2").stop();
			            tips('请输入您真实的房产价值/月供金额，要不测试结果不准确哦~');
						return false
					}
			...
			//外地房产
				...
				if(nonlocalHouseType == 32){	
					...
					if( nonlocalHouseMonthTotalPrice == '' || nonlocalHouseMonthTotalPrice > 50000 || nonlocalHouseMonthTotalPrice < 0  || $.isNumeric(nonlocalHouseMonthTotalPrice) == false ){
						$(".loan_succ_tck2").stop();
			            tips('请填写正确月供房产总价，应该在0-50000（万）之间');
						return false
					}
